...
def initialize_k_values(concentrations):
    ...
    k[3] = k_inv[1] * concentrations[2] / (concentrations[0] * concentrations[1])
    for i in range(1, 38):
        k[2*(i+1)] = k[2*i] * concentrations[i]**2 / concentrations[i+1]**2
        k_inv[i+1] = k_inv[i] * concentrations[i+1] / concentrations[i+2]
        k[2*i+3] = k[2*i+1] * concentrations[0] * concentrations[i] / (concentrations[0] * concentrations[i+1])
...
def equations(p, t, k_values):
    ...
    for i in range(1, 39):
        dpdt[1] += k_inv[i] * p[i+2] - k[2*i+1] * p[1] * p[i+1]
    dpdt[2] = k[1] * p[1]**2 + 2 * k_inv[1] * p[3] - k_inv[0] * p[2] - k[2] * p[2]**2 - k[3] * p[1] * p[2]
    for i in range(3, 40):
        dpdt[i] = k[2*i-4] * p[2]**2 + 2 * k[2*i-3] * p[1] * p[i-1] + 2 * k_inv[i-1] * p[i+1] - 3 * k_inv[i-2] * p[i] - k[2*i-2] * p[i]**2 - k[2*i-1] * p[1] * p[i]
    dpdt[40] = k[76] * p[39]**2 + 2 * k[77] * p[1] * p[39] - 3 * k_inv[38] * p[40]
    return dpdt
...
# 输出优化结果
k_result = {f"k{i}": c for i, c in enumerate(k_optimized[:78], start=0)}
k_inv_result = list(k_optimized[78:])
