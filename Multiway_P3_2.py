...
def fit_lnk_lnp(pm, k_optimized):
    ...
    popt4, _ = curve_fit(model, pm[:35], np.log(k_optimized[115:150]), maxfev=1000)
    ...
    print(f"P3参与到后续反应公式拟合参数：: a = {a4}, x = {x4}")
    ...
    # 绘制P3参与后续反应拟合
    axs[3].scatter(pm[:35], np.log(k_optimized[115:150]), label='Natural data')
    axs[3].plot(P_fit4, k_fit4, color='yellow', label=f'ln(k) = {a4:.2f} * ln(2^n)^{x4:.2f}')
    axs[3].set_xlabel('polymer')
    axs[3].set_ylabel('ln(k)')
    axs[3].legend()
    axs[3].set_title('P3 curve fitting')
    axs[3].grid(True)
...
# 调用动画函数
save_path = r"C:\Users\柴文彬\Desktop\化学动力学\多进程_P3_way2\concentration_animation.gif"
