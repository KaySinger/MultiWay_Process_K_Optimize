...
def initialize_k_values(concentrations):
    ...
    # P5参与后续反应的初始值猜测
    for i in range(31):
        k[i + 145] = 0.5 + random.uniform(0.3, 0.5) * i
    ...
    # P6参与后续反应的初始值猜测
    for i in range(29):
        k[i + 176] = 0.5 + random.uniform(0.3, 0.5) * i
...
k_optimized = result.x[:205]
k_inv_optimized = result.x[205:]
...
# 调用动画函数
save_path = r"C:\Users\柴文彬\Desktop\化学动力学\多进程P6\多进程_P6_way1\concentration_animation.gif"
