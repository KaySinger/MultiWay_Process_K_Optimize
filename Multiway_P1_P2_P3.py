...
def initialize_k_values(concentrations):
    ...
    k_inv[115] = k[116] * concentrations[2] * concentrations[3] / concentrations[6]
    k_inv[116] = k[117] * concentrations[2] * concentrations[4] / concentrations[7]
    k[118] = k[115] * concentrations[2]**2 / (concentrations[2] * concentrations[5])
    k_inv[117] = k_inv[114] *  concentrations[5] / concentrations[8]
    for i in range(1, 32):
        k[i+118] = k[i+115] * concentrations[2] * concentrations[2+i] / (concentrations[2] * concentrations[5+i])
        k_inv[i+117] = k_inv[i+114] * concentrations[i+5] / concentrations[i+8]
    return list(k) + list(k_inv)

# 定义微分方程进程1
def equations_process1(p, t, k_values):
    k = k_values[:40]
    k_inv = k_values[150:189]
    dpdt = [0] * 41
    dpdt[0] = - k[0] * p[0]
...
# 定义微分方程进程4
def equations_process4(p, t, k_values):
    k = k_values[115:150]
    k_inv = k_values[264:]
    dpdt = [0] * 41
...
def equations(p, t, k_values):
    ...
    dpdt_process4 = equations_process4(p, t, k_values)
...
def plot_concentration_curves(t, sol):
    ...
    plt.plot(t, sol[:, 1], label='p1')
    plt.legend()
    plt.xlabel('Time')
    plt.ylabel('Concentration')
    plt.title('P1 Concentration over Time')
    plt.grid(True)
    plt.show()

    plt.figure(figsize=(20, 10))
    plt.plot(t, sol[:, 2], label='p2')
    plt.legend()
    plt.xlabel('Time')
    plt.ylabel('Concentration')
    plt.title('P2 Concentration over Time')
    plt.grid(True)
    plt.show()

    plt.figure(figsize=(20, 10))
    plt.plot(t, sol[:, 3], label='p3')
    plt.legend()
    plt.xlabel('Time')
    plt.ylabel('Concentration')
    plt.title('P3 Concentration over Time')
    plt.grid(True)
    plt.show()
    ...
    plt.xlabel('Time')
    plt.ylabel('Concentration')
    plt.title('P4-P10 Concentration over Time')
    plt.grid(True)
    plt.show()
...
# 添加参数约束，确保所有k值都是非负的
bounds = [(0, 3)] * 150 + [(0, 1)] * 149  # 确保长度为 13
...
# 输出线程1优化结果
k_result1 = {f"k{i}": c for i, c in enumerate(k_optimized[:40], start=0)}
k_inv_result1 = {f"k{i}_inv": c for i, c in enumerate(k_optimized[150:189], start=1)}
print("进程1反应式的k", k_result1)
print("进程1反应式的k_inv", k_inv_result1)

# 输出线程2优化结果
k_result2 = {f"k{i}": c for i, c in enumerate(k_optimized[40:78], start=0)}
k_inv_result2 = {f"k{i}_inv": c for i, c in enumerate(k_optimized[189:227], start=0)}
print("进程2反应式的k", k_result2)
print("进程2反应式的k_inv", k_inv_result2)

# 输出线程3优化结果
k_result3 = {f"k{i}": c for i, c in enumerate(k_optimized[78:115], start=0)}
k_inv_result3 = {f"k{i}_inv": c for i, c in enumerate(k_optimized[227:264], start=0)}
print("进程3反应式的k", k_result3)
print("进程3反应式的k_inv", k_inv_result3)

# 输出线程4优化结果
k_result4 = {f"k{i}": c for i, c in enumerate(k_optimized[115:150], start=0)}
k_inv_result4 = {f"k{i}_inv": c for i, c in enumerate(k_optimized[264:], start=0)}
print("进程4反应式的k", k_result4)
print("进程4反应式的k_inv", k_inv_result4)
