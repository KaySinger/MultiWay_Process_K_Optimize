...
def initialize_k_values(concentrations):
    ...
    # P7参与后续反应的初始值猜测
    for i in range(27):
        k[i + 205] = 0.5 + random.uniform(0.3, 0.5) * i
...
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_optimized[175:204], start=0)}
print("进程6反应式的k:", k_result)
print("进程6反应式的k_inv:", k_inv_result)

# 输出线程7优化结果
k_result = {f"k{i}": c for i, c in enumerate(k_optimized[205:232], start=0)}
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_optimized[204:], start=0)}
