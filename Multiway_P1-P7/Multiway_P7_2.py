...
def initialize_k_values(concentrations):
    ...
    # P7参与后续反应的初始值猜测
    for i in range(27):
        k[i + 243] = 0.5 + random.uniform(0.3, 0.5) * i
...
k_optimized = result.x[:270]
k_inv_optimized = result.x[270:]
...
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_optimized[213:242], start=0)}
print("进程7反应式的k:", k_result)
print("进程7反应式的k_inv:", k_inv_result)

# 输出线程8优化结果
k_result = {f"k{i}": c for i, c in enumerate(k_optimized[243:270], start=0)}
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_optimized[242:], start=0)}
...
# 调用动画函数
save_path = r"C:\Users\柴文彬\Desktop\化学动力学\多进程P7\多进程_P7_way2\concentration_animation.gif"
