...
def initialize_k_values(concentrations):
    ...
    # P10参与后续反应的初始值猜测
    for i in range(21):
        k[i + 280] = 1 + random.uniform(0.3, 0.5) * i
    k_inv[279] = (k[280] * concentrations[9] ** 2) / concentrations[19]
    for i in range(20):
        k_inv[i + 280] = k[i + 281] * concentrations[9] * concentrations[i + 10] / concentrations[i + 20]
...
def equations(p, t, k, k_inv):
    ...
    dpdt_process10 = Diffusion_Special_P1.equations_process10(p, t, k, k_inv)
    dpdt = [dpdt_process1[i] + dpdt_process2[i] + dpdt_process3[i] + dpdt_process4[i] + dpdt_process5[i] + dpdt_process6[i] + dpdt_process7[i] + dpdt_process8[i]
            + dpdt_process9[i] + dpdt_process10[i] for i in range(41)]
    return dpdt
...
k_optimized = result.x[:301]
k_inv_optimized = result.x[301:]
...
# 输出线程10优化结果
k_result = {f"k{i}": c for i, c in enumerate(k_optimized[280:301], start=0)}
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_inv_optimized[279:300], start=0)}
...
# 调用动画函数
save_path = r"C:\Users\柴文彬\Desktop\化学动力学\多进程P10\多进程_P10_way1\concentration_animation.gif"
