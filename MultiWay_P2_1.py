...
def initialize_k_values(concentrations):
    ...
    # P2参与后续反应的初始值猜测
    for i in range(37):
        k[i+40] = 1 + random.uniform(0.2, 0.5) * i
...
k_optimized = result.x[:77]
k_inv_optimized = result.x[77:]
...
k_inv_result = {f"k{i}_inv": c for i, c in enumerate(k_inv_optimized[39:], start=0)}
...
# 调用动画函数
save_path = r"C:\Users\柴文彬\Desktop\化学动力学\多进程P2\多进程_P2_way1\concentration_animation.gif"
