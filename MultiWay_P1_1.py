...
def initialize_k_values(concentrations):
    ...
    k[2] = (k[1] * concentrations[0]**2) / (concentrations[0] * concentrations[1])
    k_inv[1] = (k[2] * concentrations[0] * concentrations[1]) / concentrations[2]
    for i in range(2, 39):
        k[i+1] = (k[i] * concentrations[0] * concentrations[i-1]) / (concentrations[0] * concentrations[i])
        k_inv[i] = k_inv[i-1] * concentrations[i] / concentrations[i+1]
